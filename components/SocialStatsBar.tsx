...
export default function SocialStatsBar() {
    ...
    const fetchStats = async () => {
        try {
            const res = await axios.get('/api/social/stats');
            setStats(res.data);
        } catch (error) {
    ...
    if (loading) {
        return (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center justify-center">
                <div className="text-slate-400 text-sm">Loading social stats...</div>
            </div>
        );
    }

    if (!stats || (!stats.facebook && !stats.instagram)) {
            ...
            <div className="flex items-center justify-center space-x-8">
                {stats.facebook && (
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <div className="text-xs text-slate-400 uppercase tracking-wider">Facebook</div>
                            <div className="text-lg font-bold text-white">
                                {stats.facebook.followers.toLocaleString()} Followers
                            </div>
                        </div>
                    </div>
                )}
                ...
                {stats.instagram && (
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <div className="text-xs text-slate-400 uppercase tracking-wider">Instagram</div>
                            <div className="text-lg font-bold text-white">
